...
```{r}
# -----------------------------------------
# 1. Prepare the data for PCA (all predictors)
# -----------------------------------------
# We will exclude SO2 (the response) and the newly created pollution_level.
# The remaining numeric columns form our predictor matrix:
predictors <- air_data[, c("temp", "wind", "precip", "predays", "manuf", "popul")]

# Check for any missing values (should be none in USairpollution)
sum(is.na(predictors))

# Scale each variable to mean=0, sd=1 before PCA
...
# (we already scaled manually, so scale.=FALSE here)

# View a summary: eigenvalues, proportion of variance, cumulative variance
